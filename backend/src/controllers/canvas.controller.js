...
import { getAllMessagesService } from "../services/message.service.js";
...
export const placePixelController = async (req, res) => {
  ...
  try {
    ...
    const pixel = await placePixelService(pixelData, userId, pixelData.canvas);
...
export const erasePixelController = async (req, res) => {
  ...
  try {
    ...
    await erasePixelService(pixelData, canvasId);
...
export const updateCanvasColorsController = async (req, res) => {
  try {
    ...
    const updatedLobby = await updateCanvasColorsService(canvasId, colors);

    res.status(200).json({
      message: "Allowed colors updated successfully",
      allowedColors: updatedLobby.allowedColors,
    });
  } catch (error) {
    console.error("Error in updateCanvasColorsController:", error.message);
    res.status(500).json({ message: "Failed to update allowed colors" });
  }
};
